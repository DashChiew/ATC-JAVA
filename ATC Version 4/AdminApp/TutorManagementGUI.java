...
import common.model.User; // For User objects (e.g., tutor user data)
import common.model.Subject; // For Subject objects
import common.model.TutorAssignment; // For TutorAssignment objects
import common.util.FileHandler; // For file operations
import AdminApp.controller.AdminFunctionality; // For admin-related tutor operations (register, delete, display)
import AdminApp.controller.AssignmentFunctionality;
...
public class TutorManagementGUI {
    ...
    public static void showAllTutorInfoDialog(JFrame parent) {
        ...
        dialog.setSize(800, 500); // Increased size for table
        dialog.setLayout(new BorderLayout(10, 10));
        dialog.getContentPane().setBackground(new Color(40, 40, 40));
        ...
        // Populate the table model with data from the tutors list.
        for (User t : tutors) {
        ...
        JTable tutorInfoTable = new JTable(tableModel);
        tutorInfoTable.setFillsViewportHeight(true); // Make the table fill the height of its scroll pane
        tutorInfoTable.setRowHeight(25); // Set a comfortable row height
        tutorInfoTable.setFont(new Font("SansSerif", Font.PLAIN, 14)); // Set font for table content
        tutorInfoTable.setBackground(new Color(60, 60, 60)); // Dark background for table cells
        tutorInfoTable.setForeground(Color.WHITE); // White text for table cells
        tutorInfoTable.getTableHeader().setBackground(new Color(80, 80, 80)); // Darker background for table header
        tutorInfoTable.getTableHeader().setForeground(Color.WHITE); // White text for table header
        tutorInfoTable.getTableHeader().setFont(new Font("SansSerif", Font.BOLD, 14)); // Bold font for header

        // Add the table to a JScrollPane to enable scrolling if content overflows.
        JScrollPane scrollPane = new JScrollPane(tutorInfoTable);
